...
text_soil = text_graph("Soil",srt= 90, size=5)
text_rhizo = text_graph("Rhizosphere",srt= 90, size=5)
text_bacteria = text_graph("Bacteria", size=5)
text_root = text_graph("Root", srt=90, size=5)
text_fungi = text_graph("Fungi", size=5)
...
## Boxplot faceting by compartments
...
### BX-effect in RHIZOSPHERE samples (reported in Table S7)
...
### BX-effect in ROOT samples (reported in Table S7)
...
### plot as in Fig. S7 (all compartments)
...
## Table S7 | Alpha diversity by compartments
...
capture.output(pander(alpha_bac_soil_aov_sum, caption="soil samples: Shannon diversity bacteria"),
               ...
               pander(alpha_fDAT_root_aov_sum, caption="root samples: Shannon diversity fungi"),
               effect_size_fun_root,
               file="tables/Table_S7_Alpha_BX_statistics.doc")
```


## Figure S6 | Alpha diverstiy
...
p0 <- phyloseq::plot_ordination(bPHYSEQ, bPHYSEQ_PCoA_bray, shape="compartment", color="location")
...
p1 <- phyloseq::plot_ordination(fPHYSEQ, fPHYSEQ_PCoA_bray, shape="compartment", color="location")
...
pdf("figures/Fig_1_PCoA_all.pdf", width=9, height=4.5)
Fig_1
noshow <- dev.off()
```
...
#### Table S8 | PERMANOVA (soil samples)
...
bCAP_soil <- ordination_paperBX(PHYSEQ_object=bPHYSEQ_soil, 
                      ...
                      shape_values= c(19,1), color_values=level_cols_loc)
...
bCAP_soil <- bCAP_soil  + theme(legend.position="none") +
  panel_border(colour="black")

fCAP_soil <- fCAP_soil + theme(legend.position="none") + 
  panel_border(colour="black")

bCAP_rhizo <- bCAP_rhizo + theme(legend.position="none") +
  panel_border(colour="black")
 
fCAP_rhizo <- fCAP_rhizo + theme(legend.position="none") + 
  panel_border(colour="black")

bCAP_root <- bCAP_root + theme(legend.position="none") + 
  panel_border(colour="black")

fCAP_root <- fCAP_root + #theme(legend.position="none") +
  panel_border(colour="black")
 
 
p <- text_null + text_bacteria + text_fungi +
  text_soil + bCAP_soil + fCAP_soil +
  text_rhizo + bCAP_rhizo + fCAP_rhizo +
  text_root + bCAP_root + fCAP_root +
  plot_layout(ncol=3, nrow=4, heights=c(.2,1,1,1), widths = c(.2, 1.3, 1.3)) +
...
capture.output(pander(bDAT_rare_Re_dist_paov$aov.tab[1:6], caption="Bacteria"), 
               file="tables/Table_S9_PERMANOVA_background_genotype_bacteria.doc")
capture.output(pander(fDAT_rare_Re_dist_paov$aov.tab[1:6], caption="Fungi"),
               file="tables/Table_S9_PERMANOVA_background_genotype_fungi.doc")
```
...
capture.output(pander(bDAT_rare_Re_rhizo_dist_PW_adonis[,c(1:5)], caption="Bacteria"), 
               file="tables/Table_S9_PERMANOVA_PW_rhizo_background_genotype_bacteria.doc")

capture.output(pander(fDAT_rare_Re_rhizo_dist_PW_adonis[,c(1:5)], caption="Fungi"),
               file="tables/Table_S9_PERMANOVA_PW_rhizo_background_genotype_fungi.doc")
...
capture.output(pander(bDAT_rare_Re_root_dist_PW_adonis[,c(1:5)], caption="Bacteria"), 
               file="tables/Table_S9_PERMANOVA_PW_root_background_genotype_bacteria.doc")

capture.output(pander(fDAT_rare_Re_root_dist_PW_adonis[,c(1:5)], caption="Fungi"),
               file="tables/Table_S9_PERMANOVA_PW_root_background_genotype_fungi.doc")
...
bCAP_Re_rhizo <- bCAP_Re_rhizo + #theme(legend.position="none") +
  panel_border(colour="black") + labs(shape = "genetic\nbackground")
...
p <- text_null + text_bacteria + text_fungi +
  ...
  plot_layout(ncol=3, nrow=3, heights=c(.2,1,1), widths = c(.2, 1, 1)) +
  plot_layout(guides = 'collect') + plot_annotation(
  #subtitle = 'model = ~genotype*genetic background',
  #caption = 'model = ~genotype*genetic background',
  tag_levels = list(c('A', '') )) & 
...
# labels
pdf("figures/Fig_3_CAP.pdf", width=10, height=9)
p
noshow <- dev.off()
...
bCAP_Au_rhizo <- ordination_paperBX(PHYSEQ_object=bPHYSEQ_Au_rhizo, 
                      model= ~genotype, method ="CAP", 
                       color="genotype", 
                      shape_values= c(16,1),title_var=T, 
                      color_values=c("gold2","palegreen3", "seagreen2", "palegreen4"))
...
pdf("figures/Fig_4_Aurora_CAP.pdf", width=10, height=9)
p
noshow <- dev.off()
...
capture.output(pander(bDAT_rare_Au_dist_paov$aov.tab[1:6], caption="Bacteria"), 
               file="tables/Table_S11_PERMANOVA_mutants_bacteria.doc")
capture.output(pander(fDAT_rare_Au_dist_paov$aov.tab[1:6], caption="Fungi"),
               file="tables/Table_S11_PERMANOVA_mutants_fungi.doc")
...
capture.output(pander(bDAT_rare_Au_rhizo_dist_PW_adonis[,c(1:5)], caption="Rhizosphere bacteria"), 
               file="tables/Table_S11_PERMANOVA_PW_rhizo_mutants_bacteria.doc")
capture.output(pander(fDAT_rare_Au_rhizo_dist_PW_adonis[,c(1:5)], caption="Rhizosphere fungi"),
               file="tables/Table_S11_PERMANOVA_PW_rhizo_mutants_fungi.doc")
...
#### pairwise PERMANOVA Root [~ genotype]
...
capture.output(pander(bDAT_rare_Au_root_dist_PW_adonis[,c(1:5)], caption="Root bacteria"), 
               file="tables/Table_S11_PERMANOVA_PW_root_mutants_bacteria.doc")
capture.output(pander(fDAT_rare_Au_root_dist_PW_adonis[,c(1:5)], caption="Root fungi"),
               file="tables/Table_S11_PERMANOVA_PW_root_mutants_fungi.doc")
...
# percent abundance common / specific zOTUs 
...
### Fig 3B | Venn Reckenholz
```{r Venn Reckenholz, warning=F, echo=F}

## plot
pdf("figures/Fig_3B_venn_percentSeq.pdf", width=12, height=2.2)

par(mfrow=c(1,5), mar=c(0.2, 0.2, 1.2, 0.2))
Fig_3B_venn_rhizo_bacteria <- venndiagram(bEDGER_Re_rhizo_fit_genotype_lrt_table_OTUs, 
                                          bEDGER_Re_rhizo_fit_background_lrt_table_OTUs,
                         type="2", xpd=T, printsub=F, 
                         lines=c("red4", "steelblue3"),
                         #tcol=c( "purple3", "red4",  "steelblue3"), 
                         lcol = c("red4", "steelblue3"), # potato label
                         lwd=2, cex=1.2,
                         labels=c(paste(round(geno_percent_brhizo,1),"% cRA", 
                                        sep=""),paste(round(background_percent_brhizo,1),"% cRA", sep="")),
                         title="Rhizosphere bacteria", font.main=1, cex.main=1.5
                        ); grid.echo()

Fig_3B_venn_rhizo_fungi <- venndiagram(fEDGER_Re_rhizo_fit_genotype_lrt_table_OTUs, 
                                       fEDGER_Re_rhizo_fit_background_lrt_table_OTUs,
                         type="2", xpd=T, printsub=F, 
                         lines=c("red4", "steelblue3"),
                         #tcol=c( "purple3", "red4",  "steelblue3"), 
                         lcol = c("red4", "steelblue3"), # potato label
                         lwd=2, cex=1.2,
                         labels=c(paste(round(geno_percent_frhizo,1),"% cRA", 
                                        sep=""),paste(round(background_percent_frhizo,1),"% cRA", sep="")),
                         title="Rhizosphere fungi", font.main=1, cex.main=1.5
                        ); grid.echo()

Fig_3B_venn_root_bacteria <- venndiagram(bEDGER_Re_root_fit_genotype_lrt_table_OTUs, 
                                         bEDGER_Re_root_fit_background_lrt_table_OTUs,
                         type="2", xpd=T, printsub=F, 
                         lines=c("red4", "steelblue3"),
                         #tcol=c( "purple3", "red4",  "steelblue3"), 
                         lcol = c("red4", "steelblue3"), # potato label
                         lwd=2, cex=1.2,
                         labels=c(paste(round(geno_percent_broot,1),"% cRA", 
                                        sep=""),paste(round(background_percent_broot,1),"% cRA", sep="")),
                         title="Root bacteria", font.main=1, cex.main=1.5
                        ); grid.echo()

Fig_3B_venn_root_fungi <- venndiagram(fEDGER_Re_root_fit_genotype_lrt_table_OTUs, 
                                      fEDGER_Re_root_fit_background_lrt_table_OTUs,
                         type="2", xpd=T, printsub=F, 
                         lines=c("red4", "steelblue3"),
                         #tcol=c( "purple3", "red4",  "steelblue3"), 
                         lcol = c("red4", "steelblue3"), # potato label
                         lwd=2, cex=1.2,
                         labels=c(paste(round(geno_percent_froot,1),"% cRA", 
                                        sep=""),paste(round(background_percent_froot,1),"% cRA", sep="")),
                         title="Root fungi", font.main=1, cex.main=1.5
                        ); grid.echo()


# legend of sample PCoA
cols <- c("black", "red4", "steelblue", "black")
names(cols) <- c("cRA = cumulative rel. abundance","- genotype (WT - bx1)", "- genetic background (B73 - W22)", "Number of OTUs differing between")
plot(c(0, 1), c(0, 1), ann = F, bty = 'n', type = 'n', xaxt = 'n', yaxt = 'n')
text(rep(0,1), c(0.8), names(cols)[4], col=cols[4], adj=0,cex=1.2)
text(rep(0,1), c(0.7), names(cols)[3], col=cols[3], adj=0,cex=1.2)
text(rep(0,1), c(0.6), names(cols)[2], col=cols[2], adj=0,cex=1.2)
text(rep(0,1), c(0.4), names(cols)[1], col=cols[1], adj=0,cex=1.2)
...
x <- venndiagram(bEDGER_Au_rhizo_fit_bx1_lrt_table_OTUs, bEDGER_Au_rhizo_fit_bx2_lrt_table_OTUs, bEDGER_Au_rhizo_fit_bx6_lrt_table_OTUs, 
                         ...
                         lcol=c("palegreen3", "seagreen2", "palegreen4"), lines=c("palegreen3", "seagreen2", "palegreen4"), labels=c(paste("WT-bx1 (", bEDGER_Au_rhizo_fit_bx1_lrt_table_OTUs_percent, "%)", sep=""), paste("WT-bx2 (", bEDGER_Au_rhizo_fit_bx2_lrt_table_OTUs_percent, "%)", sep=""), paste("WT-bx6 (", bEDGER_Au_rhizo_fit_bx6_lrt_table_OTUs_percent, "%)", sep="")), title="Rhizosphere bacteria")
...
x <- venndiagram(bEDGER_Au_rhizo_fit_bx1_lrt_table_OTUs, bEDGER_Au_rhizo_fit_bx2_lrt_table_OTUs, bEDGER_Au_rhizo_fit_bx6_lrt_table_OTUs, 
type="3", xpd=T, printsub=F, tcol="black", diacol="black", lcol=c("palegreen3", "seagreen2", "palegreen4"), lines=c("palegreen3", "seagreen2", "palegreen4"),
...
write.csv(rbind(bEDGER_Au_rhizo_fit_bx1_lrt_table[,c(1:7,11:17)],
                ...
                bEDGER_Au_rhizo_fit_bx6_lrt_table[,c(1:7,11:17)]), "tables/Table_S12_edgeR_Au_rhizo_bacteria.csv")
...
x <- venndiagram(fEDGER_Au_rhizo_fit_bx1_lrt_table_OTUs, fEDGER_Au_rhizo_fit_bx2_lrt_table_OTUs, fEDGER_Au_rhizo_fit_bx6_lrt_table_OTUs, 
            type="3", xpd=T, printsub=F, tcol="black", diacol="black", 
            lcol=c("palegreen3", "seagreen2", "palegreen4"), lines=c("palegreen3", "seagreen2", "palegreen4"),
            labels=c(paste("WT-bx1 (", fEDGER_Au_rhizo_fit_bx1_lrt_table_OTUs_percent, "%)", sep=""),
                     paste("WT-bx2 (", fEDGER_Au_rhizo_fit_bx2_lrt_table_OTUs_percent, "%)", sep=""),
                     paste("WT-bx6 (", fEDGER_Au_rhizo_fit_bx6_lrt_table_OTUs_percent, "%)", sep="")), title="Rhizosphere fungi")
...
write.csv(rbind(fEDGER_Au_rhizo_fit_bx1_lrt_table[,c(1:7,11:17)],
                ...
                fEDGER_Au_rhizo_fit_bx6_lrt_table[,c(1:7,11:17)]), "tables/Table_S12_edgeR_Au_rhizo_fungi.csv")
...
# source("functions/vennDia.R")
par(mar=c(1,1,1,1))
Au_root_bacteria_venn <- venndiagram(bEDGER_Au_root_fit_bx1_lrt_table_OTUs, bEDGER_Au_root_fit_bx2_lrt_table_OTUs, bEDGER_Au_root_fit_bx6_lrt_table_OTUs, 
                                   type="3", xpd=T, printsub=F, tcol="black", diacol="black", 
                                   lcol=c("palegreen3", "seagreen2", "palegreen4"), lines=c("palegreen3", "seagreen2", "palegreen4"),
                                   labels=c("WT-bx1","WT-bx2","WT-bx6"), title="")
...
write.csv(rbind(bEDGER_Au_root_fit_bx1_lrt_table[,c(1:7,11:17)],
                ...
                bEDGER_Au_root_fit_bx6_lrt_table[,c(1:7,11:17)]), "tables/Table_S12_edgeR_Au_root_bacteria.csv")
```
...
# source("functions/vennDia.R")
par(mar=c(1,1,1,1))
Au_root_fungi_venn <- venndiagram(fEDGER_Au_root_fit_bx1_lrt_table_OTUs, fEDGER_Au_root_fit_bx2_lrt_table_OTUs, fEDGER_Au_root_fit_bx6_lrt_table_OTUs, type="3", xpd=T, printsub=F, tcol="black", diacol="black", lcol=c("palegreen3", "seagreen2", "palegreen4"), lines=c("palegreen3", "seagreen2", "palegreen4"), labels=c("WT-bx1","WT-bx2","WT-bx6"), title="")
...
write.csv(rbind(fEDGER_Au_root_fit_bx1_lrt_table[,c(1:7,11:17)],
                ...
                fEDGER_Au_root_fit_bx6_lrt_table[,c(1:7,11:17)]), "tables/Table_S12_edgeR_Au_root_fungi.csv")

```

### Figure 4B | Venn Aurora
```{r Aurora Figure 4B Venn, fig.height=3, fig.width=3, warning=F, echo=F}


pdf("figures/Fig_4B_venn_rhizo_bacteria_percentSeq.pdf", width=12, height=2.2)
par(mfrow=c(1,5), mar=c(0.2, 0.2, 1.2, 0.2))


x <- venndiagram(bEDGER_Au_rhizo_fit_bx1_lrt_table_OTUs, bEDGER_Au_rhizo_fit_bx2_lrt_table_OTUs, bEDGER_Au_rhizo_fit_bx6_lrt_table_OTUs, 
            type="3", xpd=T, printsub=F, tcol="black", diacol="black", 
            lcol=c("palegreen3", "seagreen2", "palegreen4"), 
            lines=c("palegreen3", "seagreen2", "palegreen4"),
            labels=c(paste(round(bEDGER_Au_rhizo_fit_bx1_lrt_sumPercent,1), "% cRA", sep=""), 
                     paste(round(bEDGER_Au_rhizo_fit_bx2_lrt_sumPercent,1), "% cRA", sep=""),
                     paste(round(bEDGER_Au_rhizo_fit_bx6_lrt_sumPercent,1), "% cRA", sep="")), 
            title="Rhizosphere bacteria", font.main=1, cex.main=1.5, cex=1.2)


x <- venndiagram(fEDGER_Au_rhizo_fit_bx1_lrt_table_OTUs, fEDGER_Au_rhizo_fit_bx2_lrt_table_OTUs, fEDGER_Au_rhizo_fit_bx6_lrt_table_OTUs, 
            type="3", xpd=T, printsub=F, tcol="black", diacol="black", 
            lcol=c("palegreen3", "seagreen2", "palegreen4"), lines=c("palegreen3", "seagreen2", "palegreen4"),
            labels=c(paste(round(fEDGER_Au_rhizo_fit_bx1_lrt_sumPercent,1), "% cRA", sep=""),
                     paste(round(fEDGER_Au_rhizo_fit_bx2_lrt_sumPercent,1), "% cRA", sep=""),
                     paste(round(fEDGER_Au_rhizo_fit_bx6_lrt_sumPercent,1), "% cRA", sep="")), 
            title="Rhizosphere fungi", font.main=1, cex.main=1.5, cex=1.2)
...
x <- venndiagram(bEDGER_Au_root_fit_bx1_lrt_table_OTUs, bEDGER_Au_root_fit_bx2_lrt_table_OTUs, bEDGER_Au_root_fit_bx6_lrt_table_OTUs, 
            type="3", xpd=T, printsub=F, tcol="black", diacol="black", 
            lcol=c("palegreen3", "seagreen2", "palegreen4"), lines=c("palegreen3", "seagreen2", "palegreen4"),
            labels=c(paste(round(bEDGER_Au_root_fit_bx1_lrt_sumPercent,1), "% cRA", sep=""),
                     paste(round(bEDGER_Au_root_fit_bx2_lrt_sumPercent,1), "% cRA", sep=""),
                     paste(round(bEDGER_Au_root_fit_bx6_lrt_sumPercent,1), "% cRA", sep="")), 
            title="Root bacteria", font.main=1, cex.main=1.5, cex=1.2)
...
x <- venndiagram(fEDGER_Au_root_fit_bx1_lrt_table_OTUs, fEDGER_Au_root_fit_bx2_lrt_table_OTUs, fEDGER_Au_root_fit_bx6_lrt_table_OTUs, 
            type="3", xpd=T, printsub=F, tcol="black", diacol="black", 
            lcol=c("palegreen3", "seagreen2", "palegreen4"), lines=c("palegreen3", "seagreen2", "palegreen4"),
            labels=c(paste(round(fEDGER_Au_root_fit_bx1_lrt_sumPercent,1), "% cRA", sep=""),
                     paste(round(fEDGER_Au_root_fit_bx2_lrt_sumPercent,1), "% cRA", sep=""),
                     paste(round(fEDGER_Au_root_fit_bx6_lrt_sumPercent,1), "% cRA", sep="")), 
            title="Root fungi", font.main=1, cex.main=1.5, cex=1.2)


# legend of sample PCoA
cols <- c("black", "palegreen3", "seagreen2", "palegreen4","black")
names(cols) <- c("Number of OTUs differing between\ngenotypes", "- WT-bx1", "- WT-bx2", "- WT-bx6", "cRA = cumulative rel. abundance")
## grab the scene as a grid object
library(gridGraphics)
library(grid)
plot(c(0, 1), c(0, 1), ann = F, bty = 'n', type = 'n', xaxt = 'n', yaxt = 'n')
text(rep(0,1), c(0.8), names(cols)[1], col=cols[1], adj=0, cex=1.2)
text(rep(0,4), c(0.6,0.5,0.4,0.2), names(cols)[2:5], col=cols[2:5], adj=0,cex=1.2)
...
```{r Venn all locations rhizo bacteria WT vs bx1 family level rev 1, fig.height=3, fig.width=6, warning=F, echo=F, results="hide"}
bEDGER_Ch_rhizo_BX_fit_lrt_table_fams_enriched <- unique(bEDGER_Ch_rhizo_BX_fit_lrt_table$family[bEDGER_Ch_rhizo_BX_fit_lrt_table$logFC>0])
bEDGER_Ch_rhizo_BX_fit_lrt_table_fams_depleted <- unique(bEDGER_Ch_rhizo_BX_fit_lrt_table$family[bEDGER_Ch_rhizo_BX_fit_lrt_table$logFC<0])
...
```{r Fig S7 MAplot all locations rhizo bacteria WT vs bx1, fig.height=5, fig.width=15, warning=F, echo=F, results="hide"}
# custom function fun_maplot() to plot edgeR results as MA-plots
# source("functions/fun_maplot.R")
...
pdf("figures/Fig_S7_MAplot_rhizosphere_percentSeq.pdf", width=12, height=7.6)
#par(mfrow=c(2,4), mar=c(4.5,4.5,1.6,1.6))

par(mfrow=c(3,4), mar=c(4.5,4.5,1.7,1.6))
layout.matrix <- matrix(c(1,4,8,
                          2,5,9,
                          3,6,10,
                          0,7,11
                          ), nrow = 3, ncol = 4)
layout(mat = layout.matrix,
       heights = c(0.45,1,1), # Heights of the two rows
       widths = c(1,1,1,1)) # Widths of the two columns

plot(0:1, 0:1, cex=0, xlim=c(0,1), ylim=c(0,1), frame=F, axes=F, xlab=NA, ylab=NA); text(0.5,-0.8, label=c("Changins"), cex=1.5, xpd=T) #1

plot(0:1, 0:1, cex=0, xlim=c(0,1), ylim=c(0,1), frame=F, axes=F, xlab=NA, ylab=NA); text(0.5,-0.8, label=c("Aurora"), cex=1.5, xpd=T) #2

plot(0:1, 0:1, cex=0, xlim=c(0,1), ylim=c(0,1), frame=F, axes=F, xlab=NA, ylab=NA); text(0.5,-0.8, label=c("Reckenholz"), cex=1.5, xpd=T) #3

#plot(0:1, 0:1, cex=0, xlim=c(0,1), ylim=c(0,1), frame=F, axes=F, xlab=NA, ylab=NA); text(0.5,-0.1, label=c(""), cex=1.5, xpd=T) #4

# upper panel
fun_maplot(bEDGER_Ch_rhizo_BX_fit_lrt_topTags, title="", ylab="Log fold change (M)", xlab="", ylim = ylim_ma, xlim=xlim_ma)
mtext("A", font=2, side=3, line=0.5, adj=-0.25, cex=1) #4

fun_maplot(bEDGER_Au_rhizo_BX_fit_lrt_topTags, title="", ylab=" ", xlab="", ylim = ylim_ma, xlim=xlim_ma) #5

fun_maplot(bEDGER_Re_rhizo_BX_fit_lrt_topTags, title="", ylab=" ", xlab="", ylim = ylim_ma, xlim=xlim_ma) #6

Core_rhizo_bacteria_venn <- venndiagram(bEDGER_Ch_rhizo_BX_fit_lrt_table_OTUs, bEDGER_Au_rhizo_BX_fit_lrt_table_OTUs, bEDGER_Re_rhizo_BX_fit_lrt_table_OTUs, type="3", xpd=T, printsub=F, tcol="black", diacol="black", lcol=level_cols_loc, lines=level_cols_loc, labels=c(paste( bEDGER_Ch_rhizo_BX_fit_lrt_sumPercent, "% cRA", sep=""), paste( bEDGER_Au_rhizo_BX_fit_lrt_sumPercent, "% cRA", sep=""), paste( bEDGER_Re_rhizo_BX_fit_lrt_sumPercent, "% cRA", sep="")), title="Rhizosphere bacteria", font.main=1) #7

# lower panel
fun_maplot(fEDGER_Ch_rhizo_BX_fit_lrt_topTags, title="", ylab="Log fold change (M)", xlab="Average abundance [%] (A)", ylim = ylim_ma, xlim=xlim_ma)
mtext("B",font=2, side=3, line=0.5, adj=-0.25, cex=1) #8

fun_maplot(fEDGER_Au_rhizo_BX_fit_lrt_topTags, title="", ylab=" ", xlab="Average abundance [%] (A)", ylim = ylim_ma, xlim=xlim_ma) #9

fun_maplot(fEDGER_Re_rhizo_BX_fit_lrt_topTags, title="", ylab=" ", xlab="Average abundance [%] (A)", ylim = ylim_ma, xlim=xlim_ma) #10

Core_rhizo_fungi_venn <- venndiagram(fEDGER_Ch_rhizo_BX_fit_lrt_table_OTUs, fEDGER_Au_rhizo_BX_fit_lrt_table_OTUs, fEDGER_Re_rhizo_BX_fit_lrt_table_OTUs, type="3", xpd=T, printsub=F, tcol="black", diacol="black", lcol=level_cols_loc, lines=level_cols_loc, labels=c(paste( fEDGER_Ch_rhizo_BX_fit_lrt_sumPercent, "% cRA", sep=""), paste( fEDGER_Au_rhizo_BX_fit_lrt_sumPercent, "% cRA", sep=""), paste( fEDGER_Re_rhizo_BX_fit_lrt_sumPercent, "% cRA", sep="")), title="Rhizosphere fungi", font.main=1) #11
...
Ch_bacteria_venn <- venndiagram(bEDGER_Ch_rhizo_BX_fit_lrt_table_OTUs, bEDGER_Ch_root_BX_fit_lrt_table_OTUs,
                                ...
                                labels=c("Rhizosphere", "Root"), title="Bacteria", cex.main=0.7, font.main=1)

Ch_fungi_venn <- venndiagram(fEDGER_Ch_rhizo_BX_fit_lrt_table_OTUs, fEDGER_Ch_root_BX_fit_lrt_table_OTUs,
                                type="2", xpd=T, printsub=F, tcol="black", diacol="black", 
                                lcol=level_cols_loc[1], lines=level_cols_loc[1],
                                labels=c("Rhizosphere", "Root"), title="Fungi", cex.main=0.7, font.main=1)
```
...
### Figure 5ABC | MA plot and Venn

```{r Fig 5ABC MAplot and Venn all locations root bacteria and fungi WT vs bx1, fig.height=5, fig.width=15, warning=F, echo=F, results="hide"}
# custom function fun_maplot() to plot edgeR results as MA-plots
# source("functions/fun_maplot.R")
...
pdf("figures/Fig_5ABC_MAplot_root_percentSeq.pdf", width=12, height=12)
#par(mfrow=c(2,4), mar=c(4.5,4.5,1.7,1.6))
par(mfrow=c(5,4), mar=c(4.5,4.5,1.7,1.6))
layout.matrix <- matrix(c(1,4,8,12,15,
                          2,5,9,13,16,
                          3,6,10,14,17,
                          0,7,11,18,19
                          ), nrow = 5, ncol = 4)
layout(mat = layout.matrix,
       heights = c(0.45,1,1.15,0.7,0.7), # Heights of the two rows
       widths = c(1,1,1,1)) # Widths of the two columns
...
# upper panel
fun_maplot(bEDGER_Ch_root_BX_fit_lrt_topTags, ylab="Log fold change (M)", xlab="", ylim = ylim_ma, xlim=xlim_ma, title="") 
mtext("A", font=2, side=3, line=0.5, adj=-0.25, cex=1) #4

fun_maplot(bEDGER_Au_root_BX_fit_lrt_topTags, ylab=" ", xlab="", ylim = ylim_ma, xlim=xlim_ma, title="") #5

fun_maplot(bEDGER_Re_root_BX_fit_lrt_topTags, ylab=" ", xlab="", ylim = ylim_ma, xlim=xlim_ma, title="") #6

Core_root_bacteria_venn <- venndiagram(bEDGER_Ch_root_BX_fit_lrt_table_OTUs, bEDGER_Au_root_BX_fit_lrt_table_OTUs, bEDGER_Re_root_BX_fit_lrt_table_OTUs, type="3", xpd=T, printsub=F, font.main=1, tcol="black", diacol="black", lcol=level_cols_loc, lines=level_cols_loc, labels=c(paste( round(bEDGER_Ch_root_BX_fit_lrt_sumPercent,1), "% cRA", sep=""), paste( round(bEDGER_Au_root_BX_fit_lrt_sumPercent,1), "% cRA", sep=""), paste( round(bEDGER_Re_root_BX_fit_lrt_sumPercent,1), "% cRA", sep="")), title="Root bacteria") #7

# lower panel
par(mar=c(8, 4.5, 1.7, 1.6))

fun_maplot(fEDGER_Ch_root_BX_fit_lrt_topTags,ylab="Log fold change (M)", xlab="Average abundance [%] (A)", ylim = ylim_ma, xlim=xlim_ma, title="")
mtext("B",font = 2, side=3, line=0.5, adj=-0.25, cex=1) #8

fun_maplot(fEDGER_Au_root_BX_fit_lrt_topTags, ylab=" ", xlab="Average abundance [%] (A)", ylim = ylim_ma, xlim=xlim_ma, title="") #9

fun_maplot(fEDGER_Re_root_BX_fit_lrt_topTags, ylab=" ", xlab="Average abundance [%] (A)", ylim = ylim_ma, xlim=xlim_ma, title="") #10

Core_root_fungi_venn <- venndiagram(fEDGER_Ch_root_BX_fit_lrt_table_OTUs, fEDGER_Au_root_BX_fit_lrt_table_OTUs, fEDGER_Re_root_BX_fit_lrt_table_OTUs, type="3", xpd=T, printsub=F, font.main=1, tcol="black", diacol="black", lcol=level_cols_loc, lines=level_cols_loc, labels=c(paste( round(fEDGER_Ch_root_BX_fit_lrt_sumPercent,1), "% cRA", sep=""), paste( round(fEDGER_Au_root_BX_fit_lrt_sumPercent,1), "% cRA", sep=""), paste( round(fEDGER_Re_root_BX_fit_lrt_sumPercent,1), "% cRA", sep="")),  title="Root fungi") #11


# Venn rhizo-root

# upper panel mar=c(4.5,4.5,1.7,1.6))
par(mar = c(1, 4.5, 1, 1.6))
Ch_bacteria_venn <- venndiagram(bEDGER_Ch_rhizo_BX_fit_lrt_table_OTUs, bEDGER_Ch_root_BX_fit_lrt_table_OTUs, type="2", xpd=T, printsub=F, tcol="black", diacol="black", lcol=level_cols_loc[1], lines=level_cols_loc[1], labels=c("Rhizosphere", "Root"), title="Changins bacteria", font.main=1.5)
mtext("C", font=2, side=3, line=0.5, adj=-0.25, cex=1) #12

Au_bacteria_venn <- venndiagram(bEDGER_Au_rhizo_BX_fit_lrt_table_OTUs, bEDGER_Au_root_BX_fit_lrt_table_OTUs, type="2", xpd=T, printsub=F, tcol="black", diacol="black", lcol=level_cols_loc[2], lines=level_cols_loc[2], labels=c("Rhizosphere", "Root"), title="Aurora bacteria", font.main=1.5) #13

Re_bacteria_venn <- venndiagram(bEDGER_Re_rhizo_BX_fit_lrt_table_OTUs, bEDGER_Re_root_BX_fit_lrt_table_OTUs, type="2", xpd=T, printsub=F, tcol="black", diacol="black", lcol=level_cols_loc[3], lines=level_cols_loc[3], labels=c("Rhizosphere", "Root"), title="Reckenholz bacteria", font.main=1.5) #14

# lower panel
Ch_fungi_venn <- venndiagram(fEDGER_Ch_rhizo_BX_fit_lrt_table_OTUs, fEDGER_Ch_root_BX_fit_lrt_table_OTUs, type="2", xpd=T, printsub=F, tcol="black", diacol="black", lcol=level_cols_loc[1], lines=level_cols_loc[1],
labels=c("Rhizosphere", "Root"), title="Changins fungi", font.main=1.5) #15

Au_fungi_venn <- venndiagram(fEDGER_Au_rhizo_BX_fit_lrt_table_OTUs, fEDGER_Au_root_BX_fit_lrt_table_OTUs, type="2", xpd=T, printsub=F, tcol="black", diacol="black", lcol=level_cols_loc[2], lines=level_cols_loc[2], labels=c("Rhizosphere", "Root"), title="Aurora fungi", font.main=1.5) #16

Re_fungi_venn <- venndiagram(fEDGER_Re_rhizo_BX_fit_lrt_table_OTUs, fEDGER_Re_root_BX_fit_lrt_table_OTUs, type="2", xpd=T, printsub=F, tcol="black", diacol="black", lcol=level_cols_loc[3], lines=level_cols_loc[3],
labels=c("Rhizosphere", "Root"), title="Reckenholz fungi", font.main=1.5) #17




mat2 <- cbind(c(1,1), c(2, 1.5))
cols <- c("gold2", "palegreen3")
names(cols) <- c("BX-enriched", "BX-depleted")
pchlegend <- c(16,16)
plot(mat2, col=cols, pch=pchlegend, cex=3, lwd=4, xlim=c(.5,5), ylim=c(.5,3.5), frame=F, axes=F, xlab=NA, ylab=NA)
# points(mat2+0.2, col=cols, pch=4, cex=5)
text(1, 2.5, label=c("BX-sensitive OTU"), adj=0.1, cex=1.5, xpd=T)
text(rep(2, 2), c(2, 1.5), label=c("BX-enriched", "BX-depleted"), adj = 0.2, cex=1.5) #18


mat2 <- cbind(c(1,1,1), c(3.1, 2.7, 2.3))
cols <- level_cols_loc
names(cols) <- c("Changins", "Reckenholz", "Aurora")
pchlegend <- c("-")
plot(mat2, col=cols, pch=pchlegend, cex=4, lwd=4, xlim=c(.5,5), ylim=c(.5,3.5), frame=F, axes=F, xlab=NA, ylab=NA)
# points(mat2+0.2, col=cols, pch=4, cex=5)
text(1, 3.6, label=c("Location"), adj=0.2, cex=1.5, xpd=T)
text(rep(2,2,2), c(3, 2.6, 2.2), label=c("Changins", "Reckenholz", "Aurora"), adj = c(0,0), cex=1.5)
#19
...
bTAX2$cols_phyla_2[bTAX2$cols_phyla_2=='seagreen3'] <- 'chartreuse'
...
pdf("figures/Fig_Sx_plot_Bacteroi_fPatho_root.pdf", width=5, height=3.8)
print(plot_Bact_fPatho_root)
dev.off()
...
pdf("figures/Fig_forInfo_plot_Bacteroi_fPatho_rhizo.pdf", width=5, height=3.8)
print(plot_Bact_fPatho_rhizo)
dev.off()
