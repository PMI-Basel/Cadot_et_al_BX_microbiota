...
# create edgeR object with abundant OTUs and TMM normalization
...
{
  dat <- cpm(edgeR_object, normalized.lib.sizes=TRUE, log=FALSE)
...
convert.edgeR.to.datTMM <-function(edgeR_object)
{
  dat <- cpm(edgeR_object, normalized.lib.sizes = TRUE, log=FALSE)
  dat <- dat/10000 # only if log=FALSE: transform from cpm to RA in percent
}
